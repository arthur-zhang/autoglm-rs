...
fn get_system_prompt_zh() -> String {
    let today = Local::now();
...
fn get_system_prompt_en() -> String {
    let today = Local::now();
    let formatted_date = today.format("%Y-%m-%d, %A").to_string();
...
pub fn get_system_prompt(lang: Language) -> String {
    match lang {
        Language::English => get_system_prompt_en(),
        Language::Chinese => get_system_prompt_zh(),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_get_system_prompt_zh() {
        let prompt = get_system_prompt(Language::Chinese);
        assert!(prompt.contains("今天的日期是"));
        assert!(prompt.contains("智能体分析专家"));
    }

    #[test]
    fn test_get_system_prompt_en() {
        let prompt = get_system_prompt(Language::English);
        assert!(prompt.contains("The current date"));
        assert!(prompt.contains("Android operation agent"));
    }
}
